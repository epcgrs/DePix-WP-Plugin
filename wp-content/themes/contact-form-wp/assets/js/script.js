...
document.addEventListener('DOMContentLoaded', async () => {
  ...
  document.getElementById('wallet-address')?.addEventListener('input', (e) => setState({ wallet: e.target.value }));
  ...
  function setupFinalMessage() {
    ...
    const amount = (formData.desiredAmountBRL || formData.desiredAmountBRL_DPX || '').replace(/\D/g,'');
    ...
    async function initializePixSession(){
      ...
      try {
        const res = await fetch('/api/pix/start', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ amountBRL: valueBRL, network: state.network, wallet: state.wallet }) });
        ...
        if (config.enableDebugLogs) simulateLocal();
      }
    }
    ...
    function attachStatusStream(txId){
      ...
      if (window.EventSource) {
        try {
          ...
          es.onerror = () => { es.close(); startPolling(txId); };
          return;
        } catch(_){ }
      }
      startPolling(txId);
    ...
    function startPolling(txId){
      ...
      const iv = setInterval(async () => {
        attempts++;
        try {
          const r = await fetch(`/api/pix/status?txId=${encodeURIComponent(txId)}`);
        ...
        } catch(_){ if (attempts > 30) clearInterval(iv); }
      }, 2000);
    }

    function simulateLocal(){
      const fake = { txId: `SIM${Date.now().toString().slice(-6)}`, brcode: '00020126580014BR.GOV...SIMULACAO', qrImageUrl: '' };
      currentTxId = fake.txId;
      applyPayload(fake);
      setStatus('waiting');
  ...
  const formSubmitHandler = async (e) => {
    ...
    try {
      log('AJAX Call Simulation Active!'); await new Promise(resolve => setTimeout(resolve, 500));
      const fakeSuccessResult = { success: true, data: { message: 'Simulação bem-sucedida!' } };
      log('Simulated AJAX Success:', fakeSuccessResult.data); setupFinalMessage(); showStep(steps.findIndex(step => step.dataset.step === '4'));
    } catch (error) {
      log('Submission Error:'); alert('Ocorreu um erro ao enviar o formulário. Por favor, tente novamente.'); submitBtns.forEach(btn => btn.disabled = false);
    }
  };
